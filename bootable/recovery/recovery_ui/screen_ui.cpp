...
/* TwOS Core Recovery Colors (Blue/Light Blue Theme)
 * recovery dark:   #2196F3 (Primary Blue Accent)
 * recovery light:  #80DEEA (Cyan/Light Blue Info)
 * fastbootd dark:  #E65100 (Default Orange - KORUNDU)
 * fastboot light:  #FDD835 (Default Yellow - KORUNDU) */
void ScreenRecoveryUI::SetColor(UIElement e) const {
  switch (e) {
    case UIElement::BATTERY_LOW:
      if (fastbootd_logo_enabled_)
        gr_color(0xfd, 0x35, 0x35, 255); // Kırmızı (Fastbootd)
      else
        gr_color(0xDC, 0x35, 0x45); // Kırmızı (Critical Error)
      break;
    case UIElement::INFO:
      if (fastbootd_logo_enabled_)
        gr_color(0xfd, 0xd8, 0x35, 255);
      else
        gr_color(0x80, 0xDE, 0xEA, 255); // TwOS Açık Mavi (Önceki MOR F890FF yerine)
      break;
    case UIElement::HEADER:
      ...
      break;
    case UIElement::MENU:
      gr_color(0xd8, 0xd8, 0xd8, 255); // Orijinal Gri Metin (KORUNDU)
      break;
    case UIElement::MENU_SEL_BG:
    case UIElement::SCROLLBAR:
      if (fastbootd_logo_enabled_)
        gr_color(0xe6, 0x51, 0x00, 255); // Orijinal Turuncu (KORUNDU)
      else
        gr_color(0x21, 0x96, 0xF3, 255); // TwOS Mavi Vurgu (Önceki MOR 7C4DFF yerine)
      break;
    case UIElement::MENU_SEL_BG_ACTIVE:
      gr_color(0, 156, 100, 255); // Yeşil/Cyan (KORUNDU)
      break;
    case UIElement::MENU_SEL_FG:
      if (fastbootd_logo_enabled_)
        gr_color(0, 0, 0, 255); // Orijinal Siyah Metin (KORUNDU)
      else
        gr_color(0xd8, 0xd8, 0xd8, 255); // Orijinal Beyazımsı Metin (KORUNDU)
      break;
    case UIElement::LOG:
      gr_color(196, 196, 196, 255); // Gri (KORUNDU)
      break;
    case UIElement::TEXT_FILL:
      gr_color(0, 0, 0, 160); // Siyah Arkaplan (KORUNDU)
      break;
    default:
      gr_color(255, 255, 255, 255); // Varsayılan Beyaz (KORUNDU)
      break;
  }
}
